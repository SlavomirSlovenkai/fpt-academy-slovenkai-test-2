...
class CarEdit extends Component {
    ...
    componentDidMount() {
        fetch('http://localhost:4000/api/cars/'+this.props.match.params.id)
          .then(res => res.json())
          .then((data) => {
            this.initState._id = data._id;
            this.initState.brand = data.brand;
            this.initState.spz = data.spz;
 ...
 handleSubmit(event) {
     ...
     let {brand, spz, _id} = this.state;
     if (!brand || !brand.trim().length || !spz || spz.trim().length !== 7)
        return;
     fetch('http://localhost:4000/api/cars/'+_id, {
      method: 'PUT',
      body: JSON.stringify({
        brand: brand,
        spz: spz
